...
void FCharmTunnelModule::StartupModule()
{
	...
	PluginCommands->MapAction(
		FCharmTunnelCommands::Get().OpenCharmTunnelPluginWindow,
		FExecuteAction::CreateRaw(this, &FCharmTunnelModule::PluginButtonClicked),
		FCanExecuteAction());

	UToolMenus::RegisterStartupCallback(
		FSimpleMulticastDelegate::FDelegate::CreateRaw(this, &FCharmTunnelModule::RegisterMenus));

	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(CharmTunnelTabName,
	                                                  FOnSpawnTab::CreateRaw(this, &FCharmTunnelModule::OnSpawnPluginTab))
...
TSharedRef<SDockTab> FCharmTunnelModule::OnSpawnPluginTab(const FSpawnTabArgs& SpawnTabArgs)
{
	...
	FText DialogText = FText::Format(
		...
		FText::FromString(TEXT("FCharmTunnelModule::PluginButtonClicked()")),
		FText::FromString(TEXT("CharmTunnel.cpp"))
	);

	return SNew(SDockTab)
		.TabRole(ETabRole::NomadTab)
		[
			SNew(SCharmTunnelWindowPrimaryWidget)
		];
}
		...
		UToolMenu* ToolbarMenu = UToolMenus::Get()->ExtendMenu("LevelEditor.LevelEditorToolBar.PlayToolBar");
		{
			FToolMenuSection& Section = ToolbarMenu->FindOrAddSection("PluginTools");
			{
				FToolMenuEntry& Entry = Section.AddEntry(
					FToolMenuEntry::InitToolBarButton(FCharmTunnelCommands::Get().OpenCharmTunnelPluginWindow));
